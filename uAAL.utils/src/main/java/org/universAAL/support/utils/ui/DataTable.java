 ...
 * data model composed by equally structured entries, whether it큦 a table, list
 ...
 * Entry Model defining what is shown from each entry and how. The Entry Model
 * is like a Group, any Control (except Submits or other Data Tables) can be added to it.
 * Each Control added must reference a Path from the entries in Data Model, and
 * must not have an initial value. The Entry Model will be repeated for each
 * entry in the Data Model, and each of its Controls initialized with the data
 ...
 * Model. This is because the Tables큦 Entry Model is not actually created until
 ...
 * from the parent큦 root resource or setting the Data Model entries explicitly.
 * <p>
 * Example render:
 * <p>
 * 
...
public class DataTable extends Control implements IContainer{
     ...
     * 
     * @param ref
     *            The simple reference identifying the Table Data Model from
     *            the parent root. Set to null to auto-generate and not use a
     *            Data Model from the root.
     */
     ...
     * Model entry property specified by the reference. The Control큦 reference
     * path must follow a valid path starting where the Table큦 own path
     * finished. Example:
     * 
     * <pre>
